...
description: "The abstract interface that decouples the unified trainer from specific training infrastructure, enabling pluggable backends."
---
...
[unified trainer](/training/unified-trainer) from any specific training infrastructure.
...
For workflows that compute per-token advantages during episode generation
(stage 1) — for example, distillation flows that derive a reverse-KL signal
from a teacher policy — every `Step` already has its `.advantage` field set
...
<Steps>
  <Step title="Subclass BackendProtocol">
    ```python
  ...
  <Step title="Override lifecycle hooks as needed">
    ```python
