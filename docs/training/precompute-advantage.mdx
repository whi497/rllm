---
title: "Pre-computing advantage"
description: "How to set step.advantage during workflow rollout and let the unified trainer consume it directly, enabling mixed-mode training."
---

<Note>
**Related modules**: `rllm.agents.agent.Step`, `rllm.trainer.algorithms.advantage`, `rllm.trainer.algorithms.config.AlgorithmConfig`
</Note>
...
To enable precomputed advantages, set `rllm.algorithm.use_precomputed_advantage: true`.
...
`rllm.trainer.algorithms.advantage.collect_reward_and_advantage_from_trajectory_groups`.
...
has two `solver` trajectories and one `judge` trajectory, the workflow (with some simplification from `cookbooks.workflow.solver_judge.solver_judge_flow`) looks like this:
...
<Accordion title="Grouping intuition">
...
In reality, this might depend on the backend you use -- for instance, in Tinker this can be a `tinker.SamplingClient` instance.
...
- [Advantage estimator](/training/advantage-estimator)
- [Unified trainer](/training/unified-trainer)
