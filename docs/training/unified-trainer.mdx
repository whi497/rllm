---
title: "Unified trainer"
description: "The central orchestrator for backend-agnostic training in rLLM, managing the full training loop from episode generation to policy updates."
---
...
`BackendProtocol` interface (see [backend protocol](/training/backend-protocol)).

## Entry points

There are two ways to start training:

### `AgentTrainer` (recommended for standard backends)
...
<Accordion title="Training loop diagram">
```mermaid
graph TD
  n1["fit()"] --> n2["fit_async()"]
...
For the full configuration reference, see [configuration](/training/configuration).
