"""Rewind-choice MiniSweeper agent flow -- reward variant: accumulated play + reflection diff.
...
differences are entirely in reward assignment and GRPO grouping:
  ...
  - Play segments receive the path-cumulative environment reward AS-IS: each play
    segment ``i`` gets ``S_i``, the accumulated env reward at the moment that
    segment ends (its ``metadata["cum_reward_at_end"]`` snapshot). Unlike the
    ``discounted_reflect_diff`` variant there is NO discounting and NO collapsing
    of the outcome onto the final segment -- the reward a segment earns is exactly
    the cumulative env reward standing at its end. (When the episode wins, the
    final segment's accumulated reward already reflects the win via the env's
    terminal reward, so ``traj_gamma`` is unused here.)
  - Reflections receive a *fine-grained forward difference* of the path
    cumulative environment reward (same as the ``reflect_reward_diff`` and
    ``discounted_reflect_diff`` variants). Let ``S_i`` be the path-cumulative env
    reward at the moment play segment ``i`` ends; ``reflection_k`` gets
    ``S_{k+1} - S_k`` -- the extra cumulative reward the next attempt earned
    relative to where this reflection was triggered. The diff is NOT discounted;
    it uses the raw ``cum_reward_at_end`` snapshots.
...
@rllm.rollout(name="minisweeper_rewind_accumulated_reflect_diff")
async def minisweeper_rewind_accumulated_reflect_diff_flow(task: Task, config: AgentConfig) -> Episode:
