"""Rewind-choice MiniSweeper agent flow -- reward variant 3: segment-level + novelty gate.

Builds on variant 2. Identical rollout behaviour to
``minisweeper_rewind_choice_flow_refacted``; only reward assignment and grouping
differ:

  - No discounting (``traj_gamma`` is ignored).
  - Reflections: same forward cumulative-reward difference as variant 2. Let
    ``S_i`` be the path-cumulative env reward when play segment ``i`` ends;
    ``reflection_k`` gets ``S_{k+1} - S_k``.
  - Play segments: SEGMENT-LEVEL reward -- every step in a segment shares the
    same value, the terminal outcome (1.0 if won else 0.0).
  - NOVELTY GATE on play segments: if a segment's reveal actions produced NO new
    environment state (no naive board observation) relative to its baseline, that
    segment's reward is forced to 0.0. The baseline is "segment i-1's
    observations PLUS segment i's own prefix" -- i.e. the board states already
    seen along the active path from C_0 to where segment i started, together with
    every board state observed during the immediately preceding segment i-1. A
    segment that only re-treads already-seen board states earns nothing. The gate
    boolean is computed during rollout (where the full observation history is
    available) and stamped into the play trajectory's metadata.
  - All play segments share the trajectory name ``minisweeper_seg`` and all
    reflections share ``minisweeper_reflect``, so GRPO groups all play segments
    of a task together and all reflections of a task together.
"""
...
def _assign_cross_segment_rewards(
    ...
    """Variant ``segment_novelty_gate`` reward assignment.

    ``traj_gamma`` is accepted for signature compatibility but ignored (no
    discounting).

    Play segments get the SEGMENT-LEVEL terminal outcome reward (1.0 if won else
    0.0, the same value for every step in the segment), EXCEPT that a segment
    whose reveals produced no new environment state is zeroed. That novelty
    decision is computed during rollout against the "segment i-1 + segment i
    prefix" baseline and stamped into the play trajectory's
    ``metadata["produced_new_state"]`` (defaulting to True when absent so the gate
    never silently suppresses a reward it could not evaluate).

    Reflections get the forward cumulative-reward difference, identical to the
    ``reflect_reward_diff`` variant: trajectories are appended in execution order
    (seg0, reflect0, seg1, reflect1, ..., final_seg); each play trajectory carries
    ``metadata["cum_reward_at_end"] = S_i``; ``reflection_k`` (the k-th reflection,
    sitting between play segment k and k+1) gets ``S_{k+1} - S_k``. A reflection
    with no following play segment falls back to 0.0.
    ...
    NOTE: the novelty gate only changes the *play* segment reward; the reflection
    diff is always computed from the raw ``cum_reward_at_end`` values and is not
    affected by zeroing the play reward.
    """
    ...
    for traj_idx in play_indices:
        produced_new_state = bool((trajectories[traj_idx].metadata or {}).get("produced_new_state", True))
        trajectories[traj_idx].reward = final_reward if produced_new_state else 0.0
...
@rllm.rollout(name="minisweeper_rewind_segment_novelty_gate")
async def minisweeper_rewind_segment_novelty_gate_flow(task: Task, config: AgentConfig) -> Episode:
    ...
    async with session:
        ...
        # Novelty gate (variant 3): the naive board observations produced by the
        # reveals of the immediately preceding play segment (segment i-1). A
        # segment is "novel" if at least one of its reveals reaches a board state
        # not already in (this segment's path prefix) U (prev_segment_obs).
        prev_segment_obs: set[str] = set()
        ...
        while True:
            ...
            segment_events: list[BranchEvent] = []
            # Board observations produced by this segment's effective reveals.
            segment_reveal_obs: list[str] = []
            ...
            while True:
                ...
                segment_reveal_obs.append(observation)
            ...
            # Variant 3 novelty gate: did this segment's reveals reach any board
            # state not already in (segment prefix C_0..C_start) U (segment i-1's
            # observations)? The prefix is the active path before this segment ran.
            segment_prefix_obs = set(step_observations[: segment_start_position + 1])
            novelty_baseline = segment_prefix_obs | prev_segment_obs
            produced_new_state = any(obs not in novelty_baseline for obs in segment_reveal_obs)
            segment_meta = {
                "role": PLAY_ROLE,
                "segment_idx": segment_idx,
                "cum_reward_at_end": cum_reward,
                "produced_new_state": produced_new_state,
            }

            if won:
                if segment_steps:
                    trajectories.append(Trajectory(name=_segment_traj_name(traj_grouping, segment_idx), steps=segment_steps, reward=None, metadata=dict(segment_meta)))
                break
            ...
            # The segment that just ran becomes "segment i-1" for the next
            # segment's novelty baseline.
            prev_segment_obs = set(segment_reveal_obs)
