"""Rewind-choice MiniSweeper agent flow -- reward variant 2: reflection sum-reward diff.
...
differences are entirely in reward assignment and GRPO grouping:
  ...
  - Play segments receive the single terminal outcome reward (1.0 if won else
    0.0) -- the same value for every play segment.
  - Reflections receive a *fine-grained forward difference* of the path
    cumulative environment reward. Let ``S_i`` be the path-cumulative env reward
    (sum of the env's per-step fine-grained rewards) at the moment play segment
    ``i`` ends. ``reflection_i`` is produced after segment ``i`` triggers a
    rewind; its reward is ``S_{i+1} - S_i`` -- how much extra cumulative reward
    the *next* attempt (the segment that runs after this reflection's rewind)
    earned relative to the point this reflection was triggered.
  - The cumulative sum reward ``S`` follows the checkpoint path: rewinding to
    ``C_j`` rolls it back to its value at ``C_j`` (the abandoned branch's reward
    is discarded), then accumulation continues on the new branch.
  - All play segments share the trajectory name ``minisweeper_seg`` and all
    ...
    of a task together and all reflections of a task together. The reflection
    group is therefore optimised against its own diff-based reward signal,
    separate from the play group.
"""
...
@rllm.rollout(name="minisweeper_rewind_reflect_reward_diff")
async def minisweeper_rewind_reflect_reward_diff_flow(task: Task, config: AgentConfig) -> Episode:
