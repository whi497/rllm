"""Rewind-choice MiniSweeper agent flow -- reward variant 1: undiscounted final reward.

Identical rollout behaviour to ``minisweeper_rewind_choice_flow_refacted``. The
only differences are the reward-assignment policy and the GRPO grouping:

  - No discounting at all (``traj_gamma`` is ignored).
  - Every play-segment trajectory AND every reflection trajectory receives the
    single terminal outcome reward: 1.0 if the episode was won, else 0.0.
  - All play segments share the trajectory name ``minisweeper_seg`` and all
    reflections share ``minisweeper_reflect``, so GRPO groups every play segment
    of a task together and every reflection of a task together (NOT one group per
    segment index).

This is the simplest baseline: the final win/lose signal is broadcast verbatim
to every trajectory in the episode.
"""
...
def _assign_cross_segment_rewards(
    ...
    """Variant ``undiscounted_final``: terminal reward to every trajectory.

    ``traj_gamma`` is accepted for signature compatibility with the reference
    flow but is intentionally ignored -- there is no discounting here. Both play
    segments and reflections get the single terminal outcome (1.0 if won else
    0.0), so the final win/lose signal is broadcast verbatim. This is independent
    of the trajectory grouping mode.
    """
...
@rllm.rollout(name="minisweeper_rewind_undiscounted_final")
async def minisweeper_rewind_undiscounted_final_flow(task: Task, config: AgentConfig) -> Episode:
    ...
    async with session:
        ...
        while True:
            ...
            if won:
                if segment_steps:
                    trajectories.append(Trajectory(name=_segment_traj_name(traj_grouping, segment_idx), steps=segment_steps, reward=None, metadata={"role": PLAY_ROLE, "segment_idx": segment_idx}))
                break
            ...
            if total_llm_calls < max_total_turns:
                ...
                trajectories.append(
                    Trajectory(
                        ...
                        metadata={"role": REFLECT_ROLE, "segment_idx": segment_idx},
                        steps=[
                            Step(
                                chat_completions=[
