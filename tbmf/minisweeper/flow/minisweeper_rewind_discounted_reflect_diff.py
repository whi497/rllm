"""Rewind-choice MiniSweeper agent flow -- reward variant: discounted play + reflection diff.

Identical rollout behaviour to ``minisweeper_rewind_choice_flow_refacted``. The
differences are entirely in reward assignment and GRPO grouping:

  - Play segments receive the DISCOUNTED terminal reward, exactly like the
    reference refacted flow: only the final play segment carries the win/lose
    outcome (1.0 if won else 0.0), and each earlier play segment is discounted by
    ``traj_gamma`` per step toward it (segment k gets ``traj_gamma**(n-1-k)`` *
    final). This rewards earlier segments for being on a path that eventually
    won.
  - Reflections receive a *fine-grained forward difference* of the path
    cumulative environment reward (same as the ``reflect_reward_diff`` variant).
    Let ``S_i`` be the path-cumulative env reward at the moment play segment ``i``
    ends; ``reflection_k`` gets ``S_{k+1} - S_k`` -- the extra cumulative reward
    the next attempt earned relative to where this reflection was triggered. The
    diff is NOT discounted; it uses the raw ``cum_reward_at_end`` snapshots.
  - The cumulative sum reward ``S`` follows the checkpoint path: rewinding to
    ``C_j`` rolls it back to its value at ``C_j``, then accumulation continues on
    the new branch.
  - Play vs reflection is identified by ``metadata["role"]`` so reward assignment
    is independent of the trajectory grouping mode. Trajectory naming is controlled
    by ``traj_grouping`` (``merged`` default / ``per_stage``) and segments are
    always grouped separately from reflections.
"""
...
def _assign_cross_segment_rewards(
    ...
    """Variant ``discounted_reflect_diff`` reward assignment.

    Play segments get the DISCOUNTED terminal reward (same as the reference
    refacted flow): only the last play segment carries the win/lose outcome
    (1.0 if won else 0.0), and earlier play segments are discounted by
    ``traj_gamma`` per segment step toward the end. Reflections get the forward
    cumulative-reward difference ``S_{k+1} - S_k`` from each play trajectory's
    ``metadata["cum_reward_at_end"]`` snapshot (NOT discounted).
    ...
    # Discounted play rewards over play-segment order: only the final segment
    # carries the terminal outcome; earlier segments are discounted toward it.
    n_play = len(play_indices)
    if n_play > 0:
        discounted = [0.0] * n_play
        discounted[-1] = final_reward
        for order in range(n_play - 2, -1, -1):
            discounted[order] = traj_gamma * discounted[order + 1]
...
@rllm.rollout(name="minisweeper_rewind_discounted_reflect_diff")
async def minisweeper_rewind_discounted_reflect_diff_flow(task: Task, config: AgentConfig) -> Episode:
