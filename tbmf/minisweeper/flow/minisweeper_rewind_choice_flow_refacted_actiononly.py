        ...
        if relevant_events:
            ...
            for event in relevant_events:
                ...
                if event.model_response:
                    ...
                    # history cap. Take the final <action> tag verbatim; fall back
                    # to the recorded event.action when no tag is present (e.g.
                    # truncated responses) so we never leak reasoning back in.
                    action_tags = _ACTION_TAG_RE.findall(event.model_response)
                    action_only = action_tags[-1].strip() if action_tags else event.action
                    if action_only:
                        lines.append(f"  Model action: {action_only}")
...
@rllm.rollout(name="minisweeper_rewind_prefix_actiononly")
