"""Rewind-capable Sokoban agent flow -- reward variant: accumulated play + reflection diff.

Travel Back to Move Forward version.

Reward variant (mirrors ``minisweeper_rewind_accumulated_reflect_diff``):
  - Play segments receive the path-cumulative environment reward AS-IS: each play
    ...
    segment ends (its ``metadata["cum_reward_at_end"]`` snapshot). There is NO
    discounting and NO collapsing of the outcome onto the final segment -- the
    reward a segment earns is exactly the cumulative env reward standing at its
    end (``traj_gamma`` and ``won`` are unused for play segments; a win is already
    reflected in the final segment's accumulated reward via the env's terminal
    reward).
  - Reflections receive the forward cumulative-reward difference ``S_{k+1} - S_k``
    from the same ``cum_reward_at_end`` snapshots (NOT discounted), exactly like
    the ``reflect_reward_diff`` variant.
  - Play vs reflection is identified by ``metadata["role"]`` so reward assignment
    is independent of the trajectory grouping mode.
...
def _assign_cross_segment_rewards(
    ...
    like the ``reflect_reward_diff`` variant.
...
@rllm.rollout(name="sokoban_rewind_accumulated_reflect_diff")
async def sokoban_rewind_accumulated_reflect_diff_flow(task: Task, config: AgentConfig) -> Episode:
